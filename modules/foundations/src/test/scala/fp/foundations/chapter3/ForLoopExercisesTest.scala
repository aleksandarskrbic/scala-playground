...
import fp.foundations.chapter3.ForLoopExercises._
...
class ForLoopExercisesTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("sum concat") {
    forAll((first: List[Int], second: List[Int]) => assert((sum(first) + sum(second)) == sum(first ++ second)))
  }
  ...
  test("size is consistent with std library") {
    forAll((numbers: List[Int]) => assert(size(numbers) == numbers.size))
  }

  test("size concat") {
    forAll((first: List[Int], second: List[Int]) => assert((size(first) + size(second)) == size(first ++ second)))
  }
  ...
  test("wordCount all counts are strictly greater than 0") {
    forAll((words: List[String]) => assert(wordCount(words).values.forall(_ > 0)))
  }
  ...
  test("foldLeft noop") {
    forAll((numbers: List[Int]) => assert(foldLeft(numbers, List.empty[Int])(_ :+ _) == numbers))
  }

  test("sumFoldLeft consistent with sum") {
    forAll((numbers: List[Int]) => assert(sumFoldLeft(numbers) == sum(numbers)))
  }

  test("sizeFoldLeft consistent with size") {
    forAll((numbers: List[Int]) => assert(sizeFoldLeft(numbers) == size(numbers)))
  }

  test("minFoldLeft consistent with min") {
    forAll((numbers: List[Int]) => assert(minFoldLeft(numbers) == min(numbers)))
  }

  test("wordCountFoldLeft consistent with wordCount") {
    forAll((words: List[String]) => assert(wordCountFoldLeft(words) == wordCount(words)))
  }

  test("map consistent with List map") {
    forAll((numbers: List[Int], update: Int => Int) => assert(map(numbers)(update) == numbers.map(update)))
  }
  ...
  test("lastOption consistent with List lastOption") {
    forAll((numbers: List[Int]) => assert(lastOption(numbers) == numbers.lastOption))
  }
